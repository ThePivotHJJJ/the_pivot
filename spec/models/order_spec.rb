...
describe Order, type: :model do
  ...
  it "returns the order status" do
    order1 = Order.create(total: 100, status: "completed")
    order2 = Order.create(total: 100, status: "cancelled")
  ...
  it "updates the status to paid" do
    order1 = Order.create(total: 100, status: "ordered")

    expect(order1.update_status_paid).to eq order1.status
  end
end
